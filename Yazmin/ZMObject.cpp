...
_memory(memory),
_objectData(getObjectDataLocation(memory, objectNumber)),
...
bool ZMObject::getAttribute(int attribute) const
{
    ...
    int bit = attribute % 8;
    return (_memory.getByte(_objectData + byte) & (1 << (7 - bit)))
        ? true : false;
}
...
uint16_t ZMObject::getProperty(int property) const
{
    ...
    if (size == 2)
        ...
        return _memory.getWord(_memory.getHeader().getObjectTableLocation() + 2 * (property - 1));
}
...
uint16_t ZMObject::getNextProperty(int property) const
{
    ...
    if (property == 0)
    {
        ...
        nextProp = getPropertyAtAddress(addr,
                                        &nextSize,
                                        &propAddr,
                                        &nextAddr);
    }
    ...
    {
        int size = getPropertyAddressAndSize(property, &addr);
        nextProp = getPropertyAtAddress(addr + size,
                                        &nextSize,
                                        &propAddr,
                                        &nextAddr);
    }
...
int ZMObject::getPropertyAddressAndSize(int property,
                                        uint16_t *addr) const
{
    *addr = 0;
    ...
    while (true)
    {
        currNumber = getPropertyAtAddress(currAddr, &size, &propertyAddr, &nextAddr);
...
int ZMObject::getPropertyAtAddress(uint16_t addr,
                                   uint8_t *size,
                                   uint16_t *propertyAddr,
...
int ZMObject::getPropertyAtAddress(const ZMMemory& memory,
                                   uint16_t addr,
                                   uint8_t *size,
                                   uint16_t *propertyAddr,
                                   uint16_t *nextAddr)
...
uint16_t ZMObject::getObjectDataLocation(ZMMemory& memory, int objectNumber)
{
    ...
    return memory.getHeader().getObjectTableLocation()
        + defaultPropSize
        + (objectNumber - 1) * objectSize;
}

uint16_t ZMObject::getPropertyTableAddress(ZMMemory& memory, uint16_t objectDataLocation)
{
    if (memory.getHeader().getVersion() <= 3)
