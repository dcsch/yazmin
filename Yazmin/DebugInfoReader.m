...
- (NSString *)readString {
  NSString *str =
      [NSString stringWithCString:(char *)ptr
...
- (NSString *)readLine {
  ...
  return [NSString
      stringWithFormat:@"%d:%d:%d", fileNumber, lineNumber, charNumber];
}
...
- (void)readFileDBR {
  ...
  NSLog(@"FILE_DBR fileNumber: %d, includeName: %@, actualFileName: %@",
        fileNumber, includeName, actualFileName);
}
...
- (void)readClassDBR {
  ...
  NSLog(@"CLASS_DBR name: %@, defnStart: %@, defnEnd: %@", name, defnStart,
        defnEnd);
}
...
- (void)readObjectDBR {
  ...
  NSLog(@"OBJECT_DBR number: %d, name: %@, defnStart: %@, defnEnd: %@", number,
        name, defnStart, defnEnd);
}
...
- (void)readRoutineDBR {
  ...
  NSString *name = [self readString];
  // NSLog(@"ROUTINE_DBR routineNumber: %d, defnStart: %@, pcStart: 0x%x, name:
  // %@", routineNumber, defnStart, pcStart, name);
...
- (void)readLineRefDBR {
  ...
  NSLog(@"LINE_REF_DBR routineNumber: %d, numberOfSequencePoints: %d",
        routineNumber, numberOfSequencePoints);
  unsigned int i;
...
- (void)readRoutineEndDBR {
  ...
  NSLog(@"ROUTINE_END_DBR routineNumber: %d, defnEnd: %@, nextPCValue: 0x%x",
        routineNumber, defnEnd, nextPCValue);
}
