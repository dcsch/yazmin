...
void ZMStack::pushFrame(uint32_t callAddr,
                        uint32_t returnAddr,
                        int argCount,
                        int localCount,
...
uint16_t ZMStack::getArgCount() const
{
    return _entries[_fp + 4];
}
...
uint16_t ZMStack::getEntry(int index) const
{
...
uint32_t ZMStack::getCallEntry(int index) const
{
    return _calls[index];
}
