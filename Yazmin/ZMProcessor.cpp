...
ZMProcessor::ZMProcessor(ZMMemory& memory,
                         ZMStack& stack,
                         ZMIO& io,
                         ZMError& error) :
_memory(memory),
...
void ZMProcessor::advancePC()
{
    _pc += _instructionLength;
}
...
void ZMProcessor::add()
{
    ...
    setVariable(_store,
                static_cast<int16_t>(_operands[0])
...
//    if (_operands[0] > 0)
//    {
//        uint16_t addr = _memory.getObject(_operands[0]).getPropertyAddress(_operands[1]);
...
void ZMProcessor::get_prop_len()
{
    ...
    if (_operands[0] > 0)
    {
        ...
        uint16_t nextAddr;
        ZMObject::getPropertyAtAddress(_memory,
                                       addr,
                                       &size,
                                       &propertyAddr,
                                       &nextAddr);
...
void ZMProcessor::insert_obj()
{
    ...
    if (_operands[0] > 0)
    {
        ...
        if (_operands[1] > 0)
        {
            ...
            printf("Insert object: %s -> %s\n", obj.getShortName().c_str(), dest.getShortName().c_str());
...
void ZMProcessor::jin()
{
    ...
    if (_operands[0] > 0)
        branchOrAdvancePC(_memory.getObject(_operands[0]).getParent() == _operands[1]);
    else
        branchOrAdvancePC(false);
}
...
void ZMProcessor::sread()
{
    ...
    char *textBuf = reinterpret_cast<char *>(_memory.getData()) + _operands[0] + 1;
...
void ZMProcessor::aread()
{
    ...
    size_t maxLen = _memory.getByte(_operands[0]);
    size_t len = _io.input(reinterpret_cast<char *>(_memory.getData()) + _operands[0] + 2, maxLen);
    //_io.newLine();
...
void ZMProcessor::save()
{
    ...
    if (!_continuingAfterHalt)
    {
        ...
        size_t iffLen = paddedLength(cmemLen)
            + paddedLength(stksLen)
        ...
        snprintf(anno,
                 256,
                 "Version %d game, saved from Yazmin version 0.9.1",
...
void ZMProcessor::test_attr()
{
    ...
    if (_operands[0] == 0)
        ...
        branchOrAdvancePC(_memory.getObject(_operands[0])
                          .getAttribute(_operands[1]));
}
...
void ZMProcessor::tokenise()
{
    ...
    if (_operandCount > 2)
        printf("WARNING: tokenise with more that two operands NOT YET IMPLEMENTED\n");
