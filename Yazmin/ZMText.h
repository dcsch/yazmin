...
public:
  ...
  // Decode Z-characters into Zscii
  std::string decodeZCharsToZscii(const uint8_t *data, size_t &encodedLen);

  // Encode zscii into Z-characters
  void encodeZsciiToZchars(uint8_t *data, const char *zscii, size_t zsciiLen,
                           size_t encodedLen);
  ...
  void zCharToZscii(char z, std::string &str);
  ...
  std::string zsciiToUTF8(const std::string zscii);
